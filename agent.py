...
elif args.topology == "mini":
    # mini test topology
    NUM_VERTICE = 6
    NUM_EDGE = 9
    GRAPH = [{1:1,5:2},{0:1,5:3,3:5,2:4},{1:4,3:6},{2:6,1:5,5:7,4:8},{3:8,5:0},{0:2,1:3,3:7,4:0}]
    CAPACITY = [110, 110, 110, 110, 110, 110, 110, 110, 110]
else:
    print('topology' + str(args.topology) + ' not recognized')
    exit(1)
...
def main():
    ...
    # make the tensorboard log directory if it does not exist
    ...
    if args.model_exists:
        ...
        model = PPO2.load(args.ex_model_name, env=vec_netenv)
        ...
        print('n_layer' + str(args.n_layer) + ' not recognized')
        exit(1)
    
    print("Model built.")
    model.learn(total_timesteps=args.train_timestep)
